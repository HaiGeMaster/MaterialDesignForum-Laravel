...
class UserController extends Controller
{
  ...
  public static function AddUser($email, $password, $email_captcha, $username = "", $language = "")
  {
    ...
    $client_user = UserModel::where('email', '=', $client_email)->first(); // == null;
    if ($client_user == null) {
      ...
      if ($client_user == null) {
        ...
        if (CacheController::IsVaildCaptcha($client_email, $client_email_captcha)) {
          ...
          $user->language = $language || config('app.locale');
  ...
  public static function GetEmailCaptcha($email, $lang = "")
  {
    ...
    // 生成6位验证码
    // $code = CacheController::CreateCacheCode($client_email);
    $code = CacheController::CreateCaptcha($client_email);
    ...
    // 使用 Laravel Mail facade 发送邮件
    $is_send = false;
    try {
      Mail::to($client_email)->send(new SendMailCommon(
        '邮箱验证码',
        [
          'body' => '
            <p>您的验证码是：</p>
            <p style="font-size: 24px; font-weight: bold; color: #1976D2;">' . $code . '</p>
            <p>验证码有效期为60秒，请及时使用。</p>
          '
        ]
      ));
      $is_send = true;
    ...
    try {
      ...
      if ($user != null) {
        ...
        if (
          ...
          //   'token' => $token,
          // ];
          $update_user = UserModel::where('user_id', '=', $user->user_id)->update([
            'last_login_ip' => self::GetClientIP(),
            'last_login_location' => self::GetClientLocation(),
            'last_login_time' => Share::ServerTime(),
          ]);
   ...
   * @param string $email 邮箱
   * @param string $password 密码
   * @param string $email_captcha 邮箱验证码 md5
  ...
  public static function Reset($email, $password, $email_captcha)
  {
    ...
    $client_email_captcha = md5(base64_decode($email_captcha));
    $v = false;
    $client_cache = CacheController::IsVaildCaptcha($client_email, $client_email_captcha);
    //然后检查邮箱是否存在
    $Update_user = UserModel::where('email', '=', $client_email)->update([
      'password' => $client_password,
    ]);
    if ($client_cache && $Update_user) {
      $v = true;
    }
    $data = array(
      'cache' => $client_cache,
  ...
  public static function GetUsers(
    ...
    if ($type == 'recommended') {
      ...
      $followees_id = FollowModel::where('user_id', '=', TokenController::GetUserId($user_token))
      ...
      $followers_id = FollowModel::where('followable_id', '=', TokenController::GetUserId($user_token))
        ->where('followable_type', '=', 'user')
        ->orderBy($field, $order)
        ->pluck('user_id');
      ...
      if (!$is_admin) {
        ...
        if ($search_keywords != '') {
          ...
          $data = UserModel::orderBy($field, $order)
            ->paginate($per_page, ['*'], 'page', $page);
        }
      }
    ...
    if ($data['data'] != null) {
      foreach ($data['data'] as $key => $value) {
        if ($value['cover'] == null) {
          $value['cover'] = UserModel::CreateDefaultCover();
        }
      }
    }
  ...
  public static function DEV_AllUserCoverReset()
  {
    ...
    //   ]);
    // }
    $users = UserModel::where('cover', '=', null)->get();
    foreach ($users as $key => $value) {
      $cover = UserModel::CreateDefaultCover();
      UserModel::find($value->user_id)->update([
        'cover' => $cover,
  ...
  public static function UploadAvatar($user_token, $avatar)
  {
    ...
    $server_user = UserModel::find($server_user_id);
    if ($server_user != null) {
      ...
      if ($server_avatar != null) {
        $v = UserModel::find($server_user_id)->update([
    ...
    $data = array(
      'is_upload' => $v,
      'user' => self::GetUser($server_user_id, $user_token)['user'],
      //'avatar' => $avatar,
    );
    $v ? $data['user']['password'] = '' : ''; //不返回密码
    $v ? $data['user']['user_group'] = UserGroupModel::find($server_user->user_group_id) : '';
    return $data;
  }
  /**
   * 上传封面
  ...
  public static function SetUsersDisableTime($user_token, $user_ids, $disable_time = 0)
  {
    ...
    if ($is_admin) {
      $is_set = UserModel::whereIn('user_id', $user_ids)->update([
        'disable_time' => $disable_time,
      ]);

      // 使用伪删除
      // $is_set = UserModel::whereIn('user_id', $user_ids)->delete();
    }
    return [
      'is_delete' => $is_set,
    ];
