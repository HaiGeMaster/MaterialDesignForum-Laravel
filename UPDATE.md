...
## 📢 版本重磅更新 (Upgrade Focus)

为了提供更稳健的底层架构与更优秀的用户体验，MDF 近期完成了核心架构的大版本迭代：

*   **🔧 后端重构**：后端框架已全面迁移至 **Laravel**，带来了更优雅的代码结构、更强的扩展性以及完善的生态支持。
*   **⚡️ 前端状态管理升级**：MDUI2 主题已弃用 Vuex，全面采用 **Pinia** 进行状态管理，提升了数据流转效率与 TypeScript 兼容性。
*   **🔗 路由与鉴权逻辑优化**：
    *   MDUI2 与 Vuetify4 主题同步更新了登录页路由守卫逻辑。
    *   重构了 OAuth 2.0 登录回调处理流程，解决了旧版潜在的会话状态冲突问题。
*   **🗄️ 数据库结构变更（重要）**：
    *   废弃原有的 Unix 时间戳存储方式。
    *   现全面采用 **标准时间字符串**（如 `datetime` 类型）存储时间字段，提升数据可读性与 SQL 查询便利性。
    *   *注意：升级用户请务必手动批量变更时间字段数据*
