...
class UserGroupController extends Controller
{
  ...
  public static function GetUserLocationGroup($token)
  {
    ...
    if ($user_group_id != null) {
      $user_group = UserGroupModel::where('user_group_id', '=', $user_group_id)->first();
      if ($user_group != null) {
        return $user_group;
      }
    } else {
  ...
  public static function Ability(string $token, $name): bool //string|array 为了兼容7.4版本的PHP 暂时不使用变量类型或定义
  {
    ...
    try {
      $user = TokenModel::where('token', '=', $token)->first();
  ...
  public static function BeforeTime($token, $name, $ObjectCreationTimeStamp = null): bool
  {
    ...
    $user_id = TokenModel::where('token', '=', $token)->first()->user_id;
    if ($user_id != null) {
      $user_group_id = UserModel::where('user_id', '=', $user_id)->first()->user_group_id;
      $user_group = UserGroupModel::where('user_group_id', '=', $user_group_id)->first();
      if ($user_group != null) {
  ...
  public static function GetUserGroup($user_group_id, $user_token)
  {
    ...
    $data = null;
    if ($is_admin) {
      $data = UserGroupModel::where('user_group_id', '=', $user_group_id)->first();
    }
    return [
      'is_get' => $data != null,
      'user_group' => $data
  ...
  public static function UpdateUserGroupUserCount($user_group_id): bool
  {
    $user_count = UserModel::where('user_group_id', '=', $user_group_id)
      ...
      ->count();
    if ($user_count != null) {
      $is_update = UserGroupModel::where('user_group_id', '=', $user_group_id)
        // ->where('delete_time', '=', 0)
        ->first();
      if ($is_update) {
    ...
    {
        $user_group = UserGroupModel::find($user_group_id);
        if ($user_group) {
